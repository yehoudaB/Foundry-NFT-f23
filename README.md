to encode svg ```base64 -i images/happy.svg````

### deploy to anvil

    make deployMoodNft

### deploy to Sepolia

    make deployMoodNft ARGS="--network sepolia

### mint a mood nft

    cast send 0x5FbDB2315678afecb367f032d93F642f64180aa3 "mintNft()" --private-key <your private key>

### flip Mood Nft

    cast send 0x5FbDB2315678afecb367f032d93F642f64180aa3 "flipMood(uint256)" 0  --private-key;   <your private key>

# Happy svg

data:image/svg+xml;base64,


# Sad svg

data:image/svg+xml;base64,

